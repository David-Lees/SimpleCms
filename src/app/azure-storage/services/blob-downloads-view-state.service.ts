...
export class BlobDownloadsViewStateService {
  ...
  constructor(private blobStorage: BlobStorageService, private blobState: BlobSharedViewStateService, private sanitizer: DomSanitizer) {}
  ...
  private mapDownloadResponse = (filename: string, options: BlobContainerRequest): OperatorFunction<string, BlobItemDownload> => source =>
    source.pipe(
      map(url => ({
        filename,
